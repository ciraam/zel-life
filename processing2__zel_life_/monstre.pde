class Monstre {
  ...
  Monstre(String nom, int life, int atk, float posX, float posY, PImage img){
    this.nom = nom;
    this.life = life;
    this.atk = atk;
    this.posX = posX;
  ...
  // fonction déplacement monstre maison donjon
  void deplacementMonstre2(){ 
    if (this.life > 0) { 
      if (millis() - this.timerMil > 1150) {  // en milliseconde 1000 = 1s
           int chiffre = int(random(1,5));
          // println(chiffre);
           switch(chiffre) {
              case 1:
                if (testColision2(int(this.posX - this.speed), int(this.posY)) == 0){ 
                   this.posX -= this.speed;  // gauche
                   this.img = loadImage("../image/squeletteFortGauche.png");
                   if (testColision2(int(this.posX - this.speed), int(this.posY)) == 2){ 
                     //println("dégât");
                     lifePersonnage = lifePersonnage - this.atk;
                     myPersonnage.img = loadImage("../image/persoDegat.png");
              ...
              case 2: 
                if (testColision2(int(this.posX + this.speed), int(this.posY)) == 0){ 
                   this.posX +=  this.speed; // droite
                   this.img = loadImage("../image/squeletteFortDroite.png");
                   if (testColision2(int(this.posX + this.speed), int(this.posY)) == 2){ 
                     //println("dégât");
                     lifePersonnage = lifePersonnage - this.atk;
                     myPersonnage.img = loadImage("../image/persoDegat.png");
              ...
              case 3:
                if (testColision2(int(this.posX), int(this.posY - this.speed)) == 0){ 
                   this.posY -=  this.speed;  // haut
                   this.img = loadImage("../image/squeletteFortDos.png");
                   if (testColision2(int(this.posX), int(this.posY - this.speed)) == 2){ 
                     //println("dégât");
                     lifePersonnage = lifePersonnage - this.atk;
                     myPersonnage.img = loadImage("../image/persoDegat.png");
              ...
              case 4: 
                if (testColision2(int(this.posX), int(this.posY + this.speed)) == 0){ 
                    this.posY +=  this.speed;  // bas
                    this.img = loadImage("../image/squeletteFortFace.png");
                    if (testColision2(int(this.posX), int(this.posY + this.speed)) == 2){ 
                     //println("dégât");
                     lifePersonnage = lifePersonnage - this.atk;
                     myPersonnage.img = loadImage("../image/persoDegat.png");
  ...
  void afficherNomVie2() {
      fill(#FF1A1A);
      textSize(13);
      text(this.life + "/100", this.posX, this.posY - 10);
